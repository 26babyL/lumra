...
def fetch_fk_ids(conn):
    ...
    cur.execute("SELECT name, id FROM lumra_config_categories WHERE name IN ('Coffee Equipment','Merchandise','Manual Brew','Espresso Equipment','Grinder','Accessories','Electric Brew')")
    ...
    cur.execute("SELECT id FROM lumra_config_units WHERE name = 'Pcs' LIMIT 1")
    row = cur.fetchone()
...
def run_pipeline(total_target, batch_size, skip_attrs, dry_run):
    ...
    print()
    print("  📋 SQL check:")
    print("     SELECT c.name, COUNT(p.id) FROM lumra_config_categories c")
