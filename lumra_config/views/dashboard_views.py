...
# TODO[C3-LONG]: '_fetch_dashboard_data' = 203 baris (max 30). Pecah: _fetch_dashboard_data_validate(), _fetch_dashboard_data_query(), _fetch_dashboard_data_render()
# TODO[C3-LONG]: '_fetch_dashboard_data' terlalu panjang (203 baris). Pecah: _fetch_dashboard_data_validate(), _fetch_dashboard_data_build_context(), _fetch_dashboard_data_render()
# TODO[C3-LONG]: '_fetch_dashboard_data' = 211 baris (maks 30). Pecah: _fetch_dashboard_data_validate(), _fetch_dashboard_data_process(), _fetch_dashboard_data_respond()
...
# KPI FORMATTER
# ──────────────────────────────────────────────────────────────────────
 # TODO[C3-LONG]: '_build_kpis' = 41 baris (max 30). Pecah: _build_kpis_validate(), _build_kpis_query(), _build_kpis_render()

# TODO[C3-LONG]: '_build_kpis' terlalu panjang (41 baris). Pecah: _build_kpis_validate(), _build_kpis_build_context(), _build_kpis_render()
# TODO[C3-LONG]: '_build_kpis' = 49 baris (maks 30). Pecah: _build_kpis_validate(), _build_kpis_process(), _build_kpis_respond()
...
# ──────────────────────────────────────────────────────────────────────
# CHART DATA
# TODO[C3-LONG]: '_get_chart_data' = 47 baris (max 30). Pecah: _get_chart_data_validate(), _get_chart_data_query(), _get_chart_data_render()
# ──────────────────────────────────────────────────────────────────────

# TODO[C3-LONG]: '_get_chart_data' terlalu panjang (53 baris). Pecah: _get_chart_data_validate(), _get_chart_data_build_context(), _get_chart_data_render()
# TODO[C3-LONG]: '_get_chart_data' = 67 baris (maks 30). Pecah: _get_chart_data_validate(), _get_chart_data_process(), _get_chart_data_respond()
def _get_chart_data(period: str = "7d") -> dict:
 ...
 # TODO[C3-LONG]: 'dashboard_view' = 39 baris (max 30). Pecah: dashboard_view_validate(), dashboard_view_query(), dashboard_view_render()

# ──────────────────────────────────────────────────────────────────────
# VIEWS
# TODO[C3-LONG]: 'dashboard_view' terlalu panjang (39 baris). Pecah: dashboard_view_validate(), dashboard_view_build_context(), dashboard_view_render()
# ──────────────────────────────────────────────────────────────────────

@login_required
def dashboard_view(request):
    """
    Main dashboard with lazy-loaded KPI to improve page load speed.
    Heavy queries are loaded via AJAX after page loads.
    """
    # Load minimal data first - heavy queries will be loaded via AJAX
    context = {
        # Static data that loads instantly
        ...
        # KPI will be loaded via AJAX to improve page load
        "kpis": {
            "today_sales": "Rp 0",
            "sales_growth": "+0.0%",
            "sales_trend_up": True,
            ...
            "transaction_trend_up": True,
            "best_seller_name": "Loading...",
            "best_seller_count": "Loading...",
            "low_stock_count": 0,
            "monthly_growth": "+0.0%",
            "customer_retention": "0.0%",
            "avg_order_value": "Rp 0",
            "inventory_turnover": "0.0x/month",
            "total_revenue": "Rp 0",
            "profit_margin": "0.0%",
            "return_rate": "0.0%",
            "inventory_value": "Rp 0",
            "order_frequency": "0.00/month",
            "conversion_rate": "0.0%",
            "repeat_purchase_rate": "0.0%",
            "pending_orders": 0,
        }
    }
    return render(request, "lumra_pages/sales_insight/dashboard.html", context)


@login_required
# TODO[C3-LONG]: 'notification_view' = 32 baris (maks 30). Pecah: notification_view_validate(), notification_view_process(), notification_view_respond()
