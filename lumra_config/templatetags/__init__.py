...
# TODO[C3-LONG]: 'rupiah' = 33 baris (max 30). Pecah: rupiah_validate(), rupiah_query(), rupiah_render()
# TODO[C3-LONG]: 'rupiah' terlalu panjang (33 baris). Pecah: rupiah_validate(), rupiah_build_context(), rupiah_render()
# TODO[C3-LONG]: 'rupiah' = 35 baris (maks 30). Pecah: rupiah_validate(), rupiah_process(), rupiah_respond()
...
# TODO[C3-LONG]: 'rupiah_short' = 32 baris (maks 30). Pecah: rupiah_short_validate(), rupiah_short_process(), rupiah_short_respond()
def rupiah_short(value):
    ...
    if abs_n >= 1_000_000_000:
        query_result = f"{abs_n / 1_000_000_000:.1f}".rstrip('0').rstrip('.') + ' M'
    elif abs_n >= 1_000_000:
        query_result = f"{abs_n / 1_000_000:.1f}".rstrip('0').rstrip('.') + ' Jt'
    elif abs_n >= 1_000:
        query_result = f"{abs_n / 1_000:.1f}".rstrip('0').rstrip('.') + ' Rb'
    else:
        query_result = f"{abs_n:.0f}"
...
def percent(value, total):
    ...
    try:
        ...
        if t == 0:
            return Decimal('0')
        query_result = (_to_decimal(value) / t * 100).quantize(
