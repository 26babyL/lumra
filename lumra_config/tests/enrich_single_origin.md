...
def fetch_fk_ids(conn):
    ...
    cur.execute("SELECT name, id FROM _categories WHERE name IN ('House Blend','Single Origin','Single Origin Series')")
...
def run(excel_path, dry_run, batch_size, skip_attrs, skip_recipes):
    ...
    print("       WHERE category_id IN (SELECT id FROM _categories WHERE name ILIKE '%single origin%');")
