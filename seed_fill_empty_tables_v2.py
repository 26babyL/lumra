"""
seed_fill_empty_tables_v2.py
============================
Versi perbaikan — fix 3 masalah dari run sebelumnya:

  1. cashier_id FK error
     Root cause: inspectdb generate field bernama 'cashier_id' (ForeignKey).
     Django membuat DB column 'cashier_id_id'. Jadi assign integer harus pakai
     attname (model._meta.get_field('cashier_id').attname = 'cashier_id_id').
     Helper _fk_set() mendeteksi ini otomatis.

  2. Waste / Journal / PayVoucher / AP model tidak ditemukan
     Root cause: app_label salah. Semua model accounting & production
     kemungkinan di-register di app 'lumra_config' bukan 'accounting'/'production'.
     Fix: scan SEMUA app yang terdaftar di Django, cari by db_table name.

  3. orders=0 di akhir (seluruh batch error)
     Root cause: semua batch gagal karena cashier_id issue → tidak ada yang tersimpan.

Cara pakai (sama seperti v1):
  python seed_fill_empty_tables_v2.py --dry-run
  python seed_fill_empty_tables_v2.py --execute
  python seed_fill_empty_tables_v2.py --execute --section=orders
  python seed_fill_empty_tables_v2.py --execute --section=waste
  python seed_fill_empty_tables_v2.py --execute --section=journals
  python seed_fill_empty_tables_v2.py --execute --section=ap_ar
  python seed_fill_empty_tables_v2.py --execute --section=vouchers

  Sebagai management command:
  python manage.py fill_empty_tables_v2 --execute --section=orders
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# MODEL RESOLVER — cari by db_table, bukan by app_label + model_name
# Ini jauh lebih robust karena tidak perlu tahu app_label-nya
# ═══════════════════════════════════════════════════════════════════════════════

def _by_table(db_table: str):
    """Cari model Django berdasarkan db_table name. Case-insensitive."""
...
def _attname(model, field_name: str) -> str:
    """
    Kembalikan attname yang benar untuk assign integer ke FK field.

    Masalah: inspectdb kadang buat field FK bernama 'cashier_id'
    (bukan 'cashier'). Django lalu membuat:
      - field.name    = 'cashier_id'
      - field.attname = 'cashier_id_id'  <-- ini yang dipakai untuk assign int
      - field.column  = 'cashier_id_id'

    Jika field.name = 'cashier' (normal):
      - field.attname = 'cashier_id'     <-- standard

    Fungsi ini mendeteksi keduanya secara otomatis.
    """
    if model is None:
        return field_name
    try:
        f = model._meta.get_field(field_name)
        return f.attname  # Django menyimpan ini di meta
    except Exception:
        return field_name


def _fk_int(model, field_name: str, int_value) -> dict:
    """
    Buat dict {attname: int_value} yang benar untuk FK field.
    Gunakan ini sebagai pengganti {field_name: int_value}.

    Contoh:
      _fk_int(Order, 'cashier_id', 5)
      -> {'cashier_id_id': 5}   jika field_name='cashier_id' (inspectdb pattern)
      -> {'cashier_id': 5}      jika field_name='cashier' (normal pattern)
    """
    if int_value is None:
        return {}
    att = _attname(model, field_name)
    return {att: int_value}


def _print_fk_info(model):
    """Debug helper — print semua FK field dan attname-nya."""
    if model is None:
        return
...
def warm_cache(verbose=True):
    ...
    CACHE["user_ids"]  = [u.id for u in CACHE["users"]]  # pre-extract IDs
    ...
    if verbose:
        ...
        # Cetak info FK untuk debugging cashier_id
        if M["Order"]:
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 1 — stock_adjustment_reasons
# ═══════════════════════════════════════════════════════════════════════════════
...
def fill_adjustment_reasons(dry_run=False):
    ...
    if not model:
        print("  ⚠  Tabel lumra_config_stock_adjustment_reasons tidak ada di model Django — skip")
        return 0
...
def fill_product_batches(dry_run=False, limit=3000):
    ...
    if not model:
        print("  ⚠  Tabel lumra_config_product_batches tidak ditemukan di Django — skip")
        return 0
    ...
    # Pre-extract IDs untuk FK
    vendor_ids    = [v.id for v in vendors]
    warehouse_ids = [l.id for l in warehouses]

    # Deteksi attname FK untuk model ini
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 3+4+5 — orders, orderitems, payments, returns, returnitems
# ═══════════════════════════════════════════════════════════════════════════════
...
def fill_orders_payments_returns(dry_run=False):
    ...
    # ── Analisis FK fields di Order ──────────────────────────────────────────
    # Ini adalah inti fix: deteksi attname yang benar untuk setiap FK
    ...
    # Jika store_ids kosong, jangan filter (proses semua transfer)
    ...
    # ── Build data ────────────────────────────────────────────────────────────
    # Tiap entry: (order_kw, [oi_kw_list], pay_kw, maybe_return_kw, maybe_reti_kw)
    all_data    = []
    order_ctr   = 1
    pay_ctr     = 1
    ret_ctr     = 1
    ...
    for idx, trf in enumerate(transfers):
        ...
        # Skip non-store destinations (jika ada store_ids)
        ...
        for k in range(num_orders):
            ...
            for ti in ti_list:
                ...
                # Ambil price dari variant — perlu query kecil atau estimasi
                # Untuk performa kita gunakan estimasi dari cache variant
            ...
            if RNG.random() < 0.08 and M["Return"] and ret_flds:
                ...
                if ret_att_order:        pass  # akan diisi setelah order saved
    ...
    for batch_start in range(0, len(all_data), BATCH):
        ...
        try:
            with transaction.atomic():
                ...
                # 2. OrderItems — assign order_id dari saved_orders
                oi_objs = []
...
def fill_waste_records(dry_run=False):
    if not M["Waste"]:
        print("  ⚠  Tabel production_waste_records tidak ditemukan di Django — skip")
        return 0
    if not M["MatCons"]:
        print("  ⚠  Tabel production_material_consumptions tidak ditemukan di Django — skip")
        return 0
    ...
    for cons in cons_list:
        ...
        # Dapatkan timestamp dari production_order
        # Untuk efisiensi tidak query satu per satu — gunakan now() + offset
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 7 — stockopname (sudah oke dari v1, keep as-is dengan minor fix)
# ═══════════════════════════════════════════════════════════════════════════════
...
def fill_stockopname(dry_run=False):
    ...
    if existing > 0:
        print(f"  ⚠  Sudah ada {existing} sessions — skip (jalankan --section=stockopname --force untuk overwrite)")
        return 0
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 8 — journal_entries + lines
# ═══════════════════════════════════════════════════════════════════════════════

def fill_journals(dry_run=False):
    if not M["Journal"]:
        print("  ⚠  Tabel accounting_journal_entries tidak ditemukan di Django")
        print("     Pastikan model terdaftar. Jalankan: python manage.py shell")
        print("     >>> from django.apps import apps")
        print("     >>> [m._meta.db_table for m in apps.get_models() if 'journal' in m._meta.db_table]")
        return 0
    ...
    if not accs.get("cash") or not accs.get("sales"):
        print("  ⚠  Akun cash/sales tidak ditemukan di accounting_accounts — skip")
        return 0
        ...
        try:
            with transaction.atomic():
                ...
                for i, s_je in enumerate(saved_jes):
                    ...
                    # Assign JE id setelah JE tersimpan (ini kunci fix utama)
                    if line_je_att:
...
def fill_ap_ar(dry_run=False):
    ...
    if M["AP"]:
        ...
        for i, v in enumerate(vendors[:100]):
            ...
            paid_pct = RNG.choice([0, 0, 0, 0.5, 1.0])
        ...
        print("  ⚠  Tabel accounting_accounts_payable tidak ditemukan di Django — skip AP")
    ...
    if M["AR"]:
        ...
        for i, c in enumerate(sample_custs):
            ...
            paid_pct = RNG.choice([0, 0, 0.5, 1.0])
            paid     = (total * Decimal(str(paid_pct))).quantize(Decimal("0.01"))
            status   = "paid" if paid >= total else ("partial" if paid > 0 else "unpaid")
        ...
        print("  ⚠  Tabel accounting_accounts_receivable tidak ditemukan di Django — skip AR")
...
def fill_payment_vouchers(dry_run=False):
    if not M["PayVoucher"]:
        print("  ⚠  Tabel accounting_payment_vouchers tidak ditemukan di Django — skip")
        return 0
    if not M["AP"]:
...
def run(section="all", dry_run=True, verbose=True, debug_models=False):
    t0 = time_mod.time()
    print("\n" + "═" * 70)
    print("  KAFE NUSANTARA — Fill Empty Tables v2")
    print("═" * 70)
    print(f"  Mode    : {'DRY RUN' if dry_run else 'EXECUTE'}")
    print(f"  Section : {section}")

    # Debug: print semua model yang terdeteksi
    if debug_models:
        _inspect_models()

    # Print model resolution summary
    ...
    if missing:
        ...
        print("     Jalankan dengan --debug-models untuk lihat semua model terdaftar")
        print("     Kemungkinan: model belum di-migrate, atau managed=False di Meta\n")
    ...
    for sec in sections_to_run:
        ...
        if not fn:
            print(f"\n  ⚠  Section '{sec}' tidak dikenal. Pilihan: {list(SECTION_MAP.keys())}")
            continue
        print(f"\n  ▶ [{sec}]")
        try:
...
try:
    ...
    class Command(BaseCommand):
        help = "Fill empty tables v2 — fixed FK handling + model auto-detect"
