...
def category_create(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Category '{obj.name}' created successfully.")
...
def category_delete(request, pk):
    ...
    affected_recipes  = obj.recipes.count() if hasattr(obj, "recipes") else 0
...
def unit_create(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            model_instance = form.save()
            messages.success(request, f"Unit '{obj.name}' created successfully.")
...
def unit_update(request, pk):
    ...
    if request.method == "POST":
        form = UnitForm(request.POST, instance=obj)
        if form.is_valid():
            model_instance = form.save()
            messages.success(request, f"Unit '{obj.name}' updated successfully.")
            return redirect("units_list")
    else:
...
def unit_delete(request, pk):
    ...
    # TODO[C2-DRY]: Blok duplikat — extract ke function terpisah
    affected_products    = obj.product_set.count()
    affected_ingredients = obj.recipeingredient_set.count() if hasattr(obj, "recipeingredient_set") else 0
 # TODO[C2-DRY]: Blok duplikat — extract ke function terpisah
...
def vendor_update(request, pk):
    ...
    if request.method == "POST":
        form = VendorForm(request.POST, instance=obj)
        if form.is_valid():
            obj = form.save()
